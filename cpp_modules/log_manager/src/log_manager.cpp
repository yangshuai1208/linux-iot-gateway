...
LogManager &LogManager::setMaxCachedLogs(std::size_t maxCount)
{
    ...
    while (this->recentLogs_.size() >
           this->maxCachedLogs_)
    {
        this->recentLogs_.erase(
            this->recentLogs_.begin());
    }
...
std::size_t LogManager::getLogCount(LogLevel level) const
{
    const auto iterator =
        this->logCounts_.find(level);

    if (iterator == this->logCounts_.end())
    {
        return 0;
    }
...
{
    return this->logCounts_;
}

void LogManager::resetLogCounts()
{
    for (auto &entry : this->logCounts_)
    {
        entry.second = 0;
    }
}